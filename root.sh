...
##
##      * com.termux - sets Termux app package (com.termux).
##      * /data/data/com.termux/files/usr - sets the installation prefix path.
##      * /data/data/com.termux/files/home - sets the Termux home directory path.
##
...
DISTRO_PLUGINS_DIR="/sdcard/com.termux/distro/plugin"

# Base directory where script keeps runtime data.
RUNTIME_DIR="/data/data/com.termux/files/home/distros"

# Where rootfs tarballs are downloaded.
DOWNLOAD_CACHE_DIR="/sdcard/com.termux/distros/rootfs"

# Where extracted rootfs are stored.
INSTALLED_ROOTFS_DIR="/data/data/com.termux/files/home/distros"
...
DEFAULT_FAKE_KERNEL_VERSION="4.14.186+"
...
command_login() {
	...
	if ! $isolated_environment; then
		...
		if [ -d "/data/data/com.termux/files/apps" ]; then
	...
	if $use_termux_home; then
		if [ "$login_user" = "root" ]; then
			set -- "--bind=/data/data/com.termux/files/home:/root" "$@"
		else
			if [ -f "${INSTALLED_ROOTFS_DIR}/${distro_name}/etc/passwd" ]; then
				...
				set -- "--bind=/data/data/com.termux/files/home:${user_home}" "$@"
			else
				set -- "--bind=/data/data/com.termux/files/home:/home/${login_user}" "$@"
			fi
		fi
	fi
